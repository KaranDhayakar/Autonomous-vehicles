...
class Follower(Node):
    def __init__(self, topic_name='robot_move'):
        ...
        self.subscription = self.create_subscription(Int32,topic_name,self.move_callback,10)
    ...
    def move_callback(self, msg):
        ...
        if self.room is None:
            self.room = RobotRoom('follower',color=(0,255,0))# <...> If room is None then it has not been initialized, so initialize it
        if msg.data == ord('q'):
            raise SystemExit    # if we read a `q` then time to quit
        self.room.move(msg.data)
        self.room.draw_no_return_key()# <...> move according to the message
        # <...> draw the room
...
def main(args=None):
    ...
    try:
        rclpy.spin(follow)# <...> Use spin to keep your follower going
        pass
    except SystemExit:
